...
let file; //공유할 파일객체
//let fileDataChannel;// RTCDataChannel 
...
socket.on("welcome", async (userObjArr) => {
  ...
  for (let i = 0; i < length - 1; ++i) {
    try {
       ...
       */
      chObj[userObjArr[i].socketId] = newPeerConnection.createDataChannel('file');
...
const shareFile = () => {
  if (file) {
    ...
    console.log('1 :%o', count);
    
    if (count < 2) {//연결된 피어가 1명일때

      const channel = chObj[Object.keys(chObj)];
      doShareFile(channel, isFrist);

    } else {//연결된 피어가 2명 이상의 경우
...
function doShareFile(channel, isFrist) {
  ...
  if (channel.readyState === 'open') {
    ...
    file.arrayBuffer().then(buffer => {
      ...
      // const fileInfoData = {
      //   "endMessage": END_OF_FILE_MESSAGE,
      //   "filename": file.name
      // }
 ...
 */
function createFlieDownloadDom(filename) {
  try {
     //"파일 끝" 메시지를 수신하고 청크를 큰 배열 버퍼에 함께 넣는 코드
  ...
  li.innerHTML = `<li>
      ...
      <div class="message__info">
        ...
        <i class="fas fa-file fa-2x"></i>
          <a class="file__down" download=${fileName}
            href=${url}>${fileName}</a>
